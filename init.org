...
(use-package emacs
  ...
  (defun avi/set-font-size ()
  	(interactive)
  	;; Main typeface
  	(set-face-attribute 'default nil :font "Aporetic Sans Mono" :height 180)
...
(use-package doom-themes
  ...
  (custom-set-faces
   `(font-lock-variable-name-face ((t (:foreground ,(face-foreground 'default nil t))))))
...
(with-eval-after-load 'org
  ;; 2. Force the headline text to inherit from the default font face,
  ;; stripping out any automatic TODO-state coloring or background tints.
...
(with-eval-after-load 'org-agenda
  ...
  (set-face-attribute 'org-agenda-structure-filter nil
                      :height 4.0
                      :weight 'bold
                      :foreground "#f2f4f8")
...
(use-package eglot
  ...
  (eglot-snippet-expansion 'yasnippet) 
...
(defun avi/activate-venv-from-project ()
  "Activate the project's .venv if present, otherwise use system Python."
  (interactive)
  (require 'pyvenv)
  (let* ((project (project-current t))
         (venv-dir (when project
                     (expand-file-name ".venv" (project-root project)))))
    (if (and venv-dir (file-directory-p venv-dir))
        (progn
      ...
      (message "No .venv found; using system Python."))))
...
(use-package flymake
  ...
  :general
  (avi/leader-keys
    "lE" '(flymake-show-project-diagnostics :which-key "project errors"))
  (general-nmap
    "] d" '(flymake-goto-next-error :wk "next error")
    "[ d" '(flymake-goto-prev-error :wk "prev error"))
  :custom
...
(use-package org
  ...
  (defun avi/org-mode-setup ()
	...
	(set-face-attribute 'variable-pitch nil :font "Aporetic Sans" :height 220)
  	(set-face-attribute 'fixed-pitch nil :font "Aporetic Sans Mono" :height 220)
...
(use-package vulpea
  :custom
  ;; where your notes live (replaces denote-directory)
  (vulpea-db-sync-directories (list (expand-file-name "~/Documents/notes/")))
  (vulpea-default-notes-directory (expand-file-name "~/Documents/notes/"))
  :hook
  ;; notice when you edit a #+title: line, so you remember to propagate it
  (org-mode . vulpea-title-change-detection-mode)
  :general
  (avi/leader-keys
    "d f" '(vulpea-find :wk "find/create")
    "d i" '(vulpea-insert :wk "insert link")
    "d b" '(vulpea-find-backlink :wk "backlinks"))
  :config
...
(use-package consult-vulpea
  ...
  :general
  (avi/leader-keys
    "d f" '(consult-vulpea-find :wk "find file")
    "d g" '(consult-vulpea-grep :wk "search notes"))
  :config
  ;; this advises vulpea-select-from, so vulpea-find/vulpea-insert above
  ;; automatically get consult's live preview + narrowing for free
...
(use-package citar-vulpea
  ...
  (citar-vulpea-note-title-template "${author editor} :: ${title}")
  :config
  (citar-vulpea-mode))
#+end_src
...
(defun my/eglot-capf ()
  "Combine Eglot and Yasnippet completions."
  (setq-local completion-at-point-functions
              (list
               (cape-capf-super
                #'eglot-completion-at-point
